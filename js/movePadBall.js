...
mainContainer.addEventListener("mousemove", (event) => {
  ...
  if (padLeft < 0) return;
  if (padLeft > event.target.offsetWidth - pad.offsetWidth - 2) return;
...
window.addEventListener("keydown", (event) => {
  ...
  if (padLeft < 0) return;
  if (padLeft > window.innerWidth - pad.offsetWidth - 2) return;
...
const checkPadCollision = () => {
  if (getCollisionBetween(ball, pad)) {
    ...
    if (padCollisionPoint < pad.offsetLeft + pad.offsetWidth / 4) {
      ...
      padCollisionPoint >= pad.offsetLeft + pad.offsetWidth / 2 &&
      padCollisionPoint < pad.offsetLeft + (pad.offsetWidth / 4) * 3
    ) {
      ballsDirection.left = 2;
      ballsDirection.top = -Math.sqrt(14);
      console.log("3");
    ...
    while (ballTop + ball.offsetHeight > pad.offsetTop) ballTop--;
  }
};
...
const checkBrickCollision = () => {
  if (ignoreBrickCollision) return;
  for (let brick of bricks) {
    ...
    if (collision === "rtl" || collision === "ltr") ballsDirection.left *= -1;
    if (collision === "ttb" || collision === "btt") ballsDirection.top *= -1;
  }
};
...
const checkWallCollision = () => {
  ...
  if (ballLeft < 1) ballsDirection.left *= -1;
  if (ballTop < 1) ballsDirection.top *= -1;
